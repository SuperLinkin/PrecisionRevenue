...
export const insertContractSchema = createInsertSchema(contracts)
  ...
  .transform((data: any) => {
    ...
    if (data.completionPercentage && typeof data.completionPercentage === 'string') {
      data.completionPercentage = data.completionPercentage;  // Keep as string for numeric type
    }
    if (data.totalTransactionPrice && typeof data.totalTransactionPrice === 'string') {
      data.totalTransactionPrice = data.totalTransactionPrice;  // Keep as string for numeric type
    }
    if (data.discountAmount && typeof data.discountAmount === 'string') {
      data.discountAmount = data.discountAmount;  // Keep as string for numeric type
    }
...
.transform((data: any) => {
  ...
  // Keep numeric values as strings for PostgreSQL numeric type
  if (data.standaloneSellingPrice && typeof data.standaloneSellingPrice === 'string') {
    data.standaloneSellingPrice = data.standaloneSellingPrice;
  }
...
export const insertTransactionPriceAdjustmentSchema = createInsertSchema(transactionPriceAdjustments)
  .pick({
    contractId: true,
    adjustmentType: true,
    amount: true,
...
export const insertRevenueRecordSchema = createInsertSchema(revenueRecords)
  .pick({
    contractId: true,
    amount: true,
  ...
  .transform((data: any) => {
    ...
    if (data.recognitionPercentage && typeof data.recognitionPercentage === 'string') {
      data.recognitionPercentage = data.recognitionPercentage;
    }
